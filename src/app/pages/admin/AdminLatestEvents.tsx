...
import { ChevronDown, Plus, Trash2, Eye, EyeOff, Star, GripVertical, Search } from 'lucide-react';
...
const initialFeatured: FeaturedEvent[] = mockEvents.slice(0, 6).map((e, i) => ({
  eventId: e.id,
  title: e.title,
  category: e.category,
  venue: e.venue,
...
export function AdminLatestEvents() {
  const [events, setEvents] = useState<FeaturedEvent[]>(initialFeatured);
      ...
      <div className="ml-[200px]">
        ...
        <div className="p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-white text-[22px]" style={{ fontWeight: 800 }}>Latest Events</h2>
              <p className="text-white/30 text-[12px] mt-1">Manage which events appear in the "Latest Events" section on the marketplace homepage.</p>
            </div>
            <button
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#1a2744]">
                  <th className="text-left px-5 py-3 text-[10px] text-[#E52324] uppercase tracking-widest" style={{ fontWeight: 700 }}>Order</th>
                  ...
                  <th className="text-left px-5 py-3 text-[10px] text-[#E52324] uppercase tracking-widest" style={{ fontWeight: 700 }}>Date</th>
                  ...
                  <tr key={event.eventId} className={`border-b border-[#1a2744]/50 hover:bg-[#1a2744]/30 transition-colors ${!event.active ? 'opacity-40' : ''}`}>
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-[12px] text-white truncate max-w-[200px]" style={{ fontWeight: 600 }}>{event.title}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3">
                    ...
                    <td className="px-5 py-3 text-[11px] text-white/40">{event.date}</td>
                    <td className="px-5 py-3">
